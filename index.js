...
const getCartDataFromTheDatabase = async () => {
  try {
    ...
    console.log("Error Occured While fetching Cart Data from the Database using Mongoose", error);
  }
};
...
try {
    ...
    res.status(200).json({message:"Cart Data Successfully Fetched from the Database.", data: { cart: cartData } });
} catch (error) {
res.status(500).json({error:error.message});    
}
});
...
try {
    ...
    console.log("Error Occured While fetching Order Data from the Database using Mongoose",error);
}
}
...
try {
   ...
   res.status(200).json({message:"Order Data Successfully Fetched from the Database.", data: { cart: orderData } });
} catch (error) {
 res.status(500).json({error:error.message});   
}
});
